...
def assess_priority(
    ...
    text = normalize_text(
        " ".join([report.title, report.description, report.landmark])
    )
    reasons: list[str] = []
    ...
    if critical_hits:
        reasons.append(
            "Potential immediate danger was detected: "
        ...
        return "Critical", reasons, warning
    ...
    if high_hits:
        reasons.append(
            "A sensitive place or high-impact condition was detected: "
            + ", ".join(high_hits[:3])
        )
    ...
    if category in {
        ...
        return "Medium", reasons, None

    reasons.append("No immediate safety indicator was detected.")
    return "Low", reasons, None
...
def report_similarity(
    ...
    score = (0.45 * sequence_score) + (0.40 * jaccard) + (0.15 * location_score)
...
def triage_report(
    ...
    duplicate_of, duplicate_score = detect_duplicate(
        report,
        existing_reports or [],
    )

    reasoning_parts = [
        f"Category evidence score: {category_score}.",
        *priority_reasons,
    ]
    if duplicate_of:
        reasoning_parts.append(
            f"Likely duplicate of report #{duplicate_of} with "
            f"{duplicate_score:.0%} similarity."
        )
    else:
        reasoning_parts.append(
            f"No open report passed the duplicate threshold "
            f"(best similarity: {duplicate_score:.0%})."
        )
...
def build_case_markdown(
    ...
    if language.lower() == "arabic":
        duplicate = (
            f"بلاغ رقم {result.duplicate_of} بنسبة تشابه {result.duplicate_score:.0%}"
            if result.duplicate_of
            else "لا يوجد بلاغ مشابه مؤكد"
        )
        missing = (
        ...
        warning = result.emergency_warning or "لا يوجد تحذير طارئ."
        return f"""# ملخص البلاغ {report_id or ""}

## نتيجة الفرز
- التصنيف: **{result.category}**
- الأولوية: **{result.priority}**
- الجهة: **{result.department}**
- التشابه: **{duplicate}**

## سبب القرار
{result.reasoning}

## معلومات مقترحة للإضافة
{missing}
...
"""
    ...
    duplicate = (
        f"Report #{result.duplicate_of} at {result.duplicate_score:.0%} similarity"
        if result.duplicate_of
        else "No confirmed duplicate"
    )
    missing = (
        "\n".join(f"- {item}" for item in result.missing_information)
        if result.missing_information
    ...
    warning = result.emergency_warning or "No emergency warning."
    return f"""# Community Report Summary {report_id or ""}
